...
`browser-smoke.cjs` needs the existing Playwright Node package and installed Microsoft Edge. Set `NODE_PATH` to the directory containing Playwright if it is outside the project's dependencies. Optional `GAME_URL` overrides `http://localhost:5173`. Pass `original` only while the original visual is loaded; pass `rigged` for runtime bone/skin assertions. The benchmark records frame intervals rather than the composer's reset render counters.
