...
const out=path.join(root,'work/forge3d-step03');fs.mkdirSync(out,{recursive:true});
(async()=>{
 ...
 assert(model.meshes.every(m=>Math.abs(m.bounds[0][1])<.002),'rest feet near ground');
 ...
 const capture=()=>page.evaluate(()=>{
  ...
  return {state:v.userData.playerAnimation.state,chest:v.getObjectByName('chest').quaternion.toArray(),
   ...
   hips:v.getObjectByName('hips').position.toArray()};
 });
 ...
 const result={label,model,timing,animationStates:{idleStart,idleEnd,run,lightPose,heavyPose,dodgePose,pauseStart},start,moved,light:!!light,heavy:!!heavy,dodge,before,combat,errors};
