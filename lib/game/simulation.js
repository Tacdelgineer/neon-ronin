...
export class Simulation {
  ...
  reset() {
    this.mode='playing'; this.time=0; this.wave=0; this.kills=0; this.combo=0; this.best=0; this.comboTime=0;
  ...
  startWave() {
    ...
    this.spawnPack();
    this.banner=TITLES[this.wave-1]; this.sub=SUBS[this.wave-1]; this.bannerTime=3; this.event('wave',{wave:this.wave});
  }
  spawnPack() {
  ...
  collide(entity,r=.43) {
    ...
    for (const o of OBSTACLES) { const d=distance(entity,o); if(d<o.r+r){const a=Math.atan2(entity.x-o.x,entity.z-o.z);entity.x=o.x+Math.sin(a)*(o.r+r);entity.z=o.z+Math.cos(a)*(o.r+r);} }
  }
  obstructed(a,b) {
    for (const o of OBSTACLES) { const dx=b.x-a.x,dz=b.z-a.z,l=dx*dx+dz*dz; const t=clamp(((o.x-a.x)*dx+(o.z-a.z)*dz)/(l||1),0,1); if(Math.hypot(a.x+t*dx-o.x,a.z+t*dz-o.z)<o.r) return true; } return false;
  }
  attack(heavy=false) {
    const p=this.player;
    if(this.mode!=='playing'||p.dodge>0)return false;
  ...
  dodge(mx,mz) {
    const p=this.player;if(this.mode!=='playing'||p.dodgeCooldown>0)return false;
  ...
  update(dt,input={}) {
    ...
    for(const e of this.enemies){
      ...
      if(e.type==='melee'){
        if(d<1.95&&e.cooldown<=0&&!this.obstructed(e,p)){e.state='windup';e.timer=this.wave===3?.55:.72;e.attackAngle=aim;this.event('meleeWarning',{x:e.x,z:e.z});}
          ...
          for(const o of OBSTACLES){const sep=distance(e,o);if(sep<2.5){const side=e.id%2?1:-1;dx+=(e.x-o.x)/sep*(2.5-sep)*1.5+(e.z-o.z)/sep*side;dz+=(e.z-o.z)/sep*(2.5-sep)*1.5-(e.x-o.x)/sep*side;}}
    ...
    for(const shot of this.projectiles){shot.life-=dt;shot.x+=shot.vx*dt;shot.z+=shot.vz*dt;
      ...
      if(Math.abs(shot.x)>12||shot.z>12||shot.z<-12||OBSTACLES.some(o=>distance(o,shot)<o.r)){shot.life=0;this.event('projectileImpact',{x:shot.x,z:shot.z});}
    }
    ...
    if(this.enemies.length===0){
      ...
      else {if(this.nextWave===null){this.nextWave=4;this.banner='A moment of stillness';this.sub='WAVE CLEARED · VITALITY RESTORED';this.bannerTime=3;this.player.hp=Math.min(100,p.hp+22);this.projectiles=[];this.event('clear');}this.nextWave-=dt;if(this.nextWave<=0){this.startWave();this.nextWave=null;}}
    }
  }
  snapshot(){return {mode:this.mode,hp:this.player.hp,wave:this.wave,enemies:this.enemies.length+this.reserves*(WAVES[this.wave-1]?.length||0),kills:this.kills,time:this.time,combo:this.combo,dodge:this.player.dodgeCooldown,heavy:this.player.heavyCooldown,banner:this.banner,sub:this.sub,score:this.best};}
}
