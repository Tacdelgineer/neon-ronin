...
const KATANA_STOW_Z = -.15;

export async function loadPlayerModel() {
  const gltf = await new GLTFLoader().loadAsync(assetUrl('assets/models/hero-final.glb'));
  ...
  // Wear the existing scabbard outside the left hip, angled back toward the knee.
  const stowSocket = visual.getObjectByName('weapon_sheath');
  if (stowSocket) {
    stowSocket.position.x = .16;
    stowSocket.rotation.set(-.42, Math.PI, -.08);
  }
  ...
  if (visual.userData.katana) {
    // Seat the guard at the scabbard mouth rather than inserting the grip.
    visual.userData.katana.position.z = KATANA_STOW_Z;
    visual.userData.katana.visible = true;
  }
  return visual;
}
...
export function updatePlayerModel(visual, sim, dt) {
  ...
  if (katana) {
    ...
    if (socket && katana.parent !== socket) {
      socket.add(katana);
      katana.position.set(0, 0, drawn ? 0 : KATANA_STOW_Z);
    }
    katana.visible = true;
  }
...
export async function loadWeaponAttachments(visual, bindings) {
  ...
  const results = await Promise.allSettled(available.map(([, name]) =>
    new GLTFLoader().loadAsync(assetUrl(`assets/models/${name}-final.glb`))));
