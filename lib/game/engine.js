...
import { Simulation, clamp, TAU } from './simulation.js';
...
export class NeonRonin {
  constructor(mount,onState){
    ...
    this.renderer.domElement.tabIndex=0;this.renderer.domElement.setAttribute('aria-label','Combat arena. WASD move, mouse aim, left click combo, right click heavy, Space dodge, Escape pause.');mount.appendChild(this.renderer.domElement);
    ...
    const rim=new T.DirectionalLight(0x43bcd7,1.45);rim.position.set(10,10,-15);this.scene.add(rim);
    const shrineLight=new T.PointLight(0x48e8e9,32,16,2);shrineLight.position.set(0,3.5,-13);this.scene.add(shrineLight);
    for(const [x,z] of [[-8,-5],[8,-5],[-8,6],[8,6],[-12,10],[12,-11]]){const l=new T.PointLight(0xff9c45,13,7,2);l.position.set(x,3.4,z);this.scene.add(l);}
    const {world,animated}=buildWorld(this.scene);this.world=world;this.animated=animated;
  ...
  start(){
    this.audio.start();this.sim.reset();this.keys.clear();this.left=this.right=false;this.yaw=.52;this.shake=0;this.uiClock=0;
  ...
  updateCamera(dt,instant=false){
    ...
    const desired=new T.Vector3(intro?-3.5:p.x*.38,0,intro?-1.7:p.z*.35-1.2);if(intro)desired.add(new T.Vector3(-Math.cos(this.yaw)*2,0,Math.sin(this.yaw)*2));
    this.target.lerp(desired,instant?1:1-Math.exp(-dt*3.2));
    const radius=intro?31:26.5,vertical=intro?25:24;const cameraPosition=new T.Vector3(Math.sin(this.yaw)*radius,vertical,Math.cos(this.yaw)*radius).add(this.target);
  ...
  removeActor(rig){rig.root.removeFromParent();rig.root.traverse(o=>{if(o.isMesh){o.geometry.dispose();o.material.dispose();}});this.disposeEffect(rig.health);this.disposeEffect(rig.warning);this.disposeEffect(rig.line);}
  ...
  arc(x,z,angle,color,heavy=false,enemy=false){
    ...
    const edge=ring(this.scene,reach,.024,new T.MeshBasicMaterial({color:0xd5ffff,transparent:true,opacity:.95,blending:T.AdditiveBlending,depthWrite:false}),x,.91,z,true,-angle-.05,2.95);this.effects.push({mesh:edge,life:.12,total:.12,type:'fade'});
  }
  ...
  events(){for(const ev of this.sim.events){this.audio.play(ev);switch(ev.type){
    case'swing':this.arc(ev.x,ev.z,ev.angle,ev.heavy?0xb5ffff:C.cyan,ev.heavy);if(ev.heavy)this.pulse(ev.x,ev.z,C.cyan,6,.4);break;
  ...
  visuals(dt){
    ...
    for(const e of sim.enemies){let rig=this.actors.get(e.id);if(!rig)rig=this.makeEnemy(e);animateActor(rig,e,t,dt);
    ...
    for(const [id,rig] of this.actors)if(!sim.enemies.some(e=>e.id===id)){this.removeActor(rig);this.actors.delete(id);}
  ...
  dispose(){this.disposed=true;cancelAnimationFrame(this.frame);clearTimeout(this.flashTimer);for(const f of this.handlers)f();this.audio.dispose();disposePlayerModel(this.playerModel);this.scene.traverse(o=>{if(o.geometry)o.geometry.dispose();if(o.material){const mats=Array.isArray(o.material)?o.material:[o.material];for(const m of mats){m.map?.dispose();m.dispose();}}});this.composer.dispose();this.renderer.dispose();this.renderer.domElement.remove();if(import.meta.env.DEV)delete window.__neonRonin;}
}
