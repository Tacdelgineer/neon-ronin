...
export class NeonRonin {
  ...
  dispose(){this.disposed=true;cancelAnimationFrame(this.frame);clearTimeout(this.flashTimer);for(const f of this.handlers)f();this.audio.dispose();this.scene.traverse(o=>{if(o.geometry)o.geometry.dispose();if(o.material){const mats=Array.isArray(o.material)?o.material:[o.material];for(const m of mats){m.map?.dispose();m.dispose();}}});this.composer.dispose();this.renderer.dispose();this.renderer.domElement.remove();if(import.meta.env.DEV)delete window.__neonRonin;}
}
