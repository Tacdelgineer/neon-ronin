...
import { OBSTACLES, TAU } from './simulation.js';
...
export function buildWorld(scene){
  seed=8371;const world=new T.Group();scene.add(world);const animated={flames:[],holograms:[],water:[],petals:null};
  const tex=stoneTexture(),stones=Array.from({length:7},(_,i)=>material(new T.Color().setHSL(.115,.07,.25+i*.032),{map:tex}));
  ...
  const pool=material(0x124149,{metalness:.62,roughness:.25,transparent:true,opacity:.85});
  ...
  const circuit=(x,z,len,vertical=false,mat=palette.cyan,y=.013)=>box(world,vertical?.065:len,.035,vertical?len:.065,mat,x,y,z);
  for(const side of [-1,1]){
    ...
    for(let i=0;i<10;i++){const x=rand(-12,12);box(world,.028,rand(1,4),.025,palette.cyan,x,-2,side*13.88);}
  }
  // Engraved central lotus medallion with inlaid cyan arcs and an eight-petal relief.
  cylinder(world,3.25,3.3,.06,brass,0,.005,0,64);cylinder(world,3.12,3.12,.07,stoneDark,0,.025,0,64);
  ring(world,2.98,.027,palette.cyan,0,.075,0,true);ring(world,2.65,.045,brass,0,.085,0,true);ring(world,1.11,.04,brass,0,.085,0,true);
  for(let i=0;i<16;i++){const a=i*TAU/16;const block=box(world,.09,.08,.27,stone,Math.sin(a)*3.02,.09,Math.cos(a)*3.02);block.rotation.y=a;}
  for(let i=0;i<8;i++){
    const group=new T.Group();group.rotation.y=i*TAU/8;world.add(group);
  ...
  cylinder(world,.35,.4,.12,brass,0,.11,0,8);
  ...
  function candles(x,y,z,count=4){
    for(let i=0;i<count;i++){const px=x+rand(-.32,.32),pz=z+rand(-.3,.3),h=rand(.15,.45);cylinder(world,.045,.065,h,material(0xd8b879),px,y+h/2,pz,6);const flame=mesh(scene,new T.SphereGeometry(.065,5,5),glow(0xffb53d,4.5),px,y+h+.09,pz);flame.scale.y=2.5;flame.castShadow=false;animated.flames.push(flame);}
  }
  function pillar(x,z,height=3.6,broken=false){
    ...
    circuit(x,z+.59,.65,false,palette.cyan,.7);box(world,.035,1.3,.025,palette.cyan,x+.28,1.3,z+.59);
  ...
  for(const o of OBSTACLES)pillar(o.x,o.z,o.z<0?2.7:2.1,o.x<0&&o.z<0);
  for(const side of [-1,1]){
  ...
  for(let i=0;i<5;i++)box(world,7.8-i*.2,.24,3.5-i*.42,stone,0,.08+i*.23,-12.25-i*.18);
  box(world,10,1.15,4,stoneDark,0,.44,-15.1);box(world,10.5,.2,4.5,stone,0,1.08,-15.1);
  function portal(x,z,scale=1){
    ...
    const holo=new T.Group();holo.position.set(x,1.6,z);holo.scale.setScalar(scale);scene.add(holo);animated.holograms.push(holo);
    const holoMat=new T.MeshBasicMaterial({color:C.cyan,transparent:true,opacity:.7,blending:T.AdditiveBlending,depthWrite:false});
    ring(holo,.66,.022,palette.cyan,0,2.05,0);ring(holo,.78,.009,palette.cyan,0,2.05,0);
    for(let i=-3;i<=3;i++){const px=i*.35,y=2.9+rand(-.5,.7);box(holo,.045,y,.045,palette.cyan,px,y/2,0);box(holo,.2,.035,.04,palette.cyan,px,y*.74,0);}
    const diamond=ring(holo,1.04,.012,palette.cyan,0,2.05,0);diamond.scale.set(.8,1.2,1);
    ...
    cylinder(holo,1.65,1.65,.035,palette.cyan,0,0,0,4);cylinder(holo,1.57,1.57,.05,palette.dark,0,.01,0,4);
  }
  portal(0,-14.8,1.1);
  ...
  for(const s of [-1,1]){
    pillar(s*5.7,-14,7);banner(s*5.7,4.35,-13.32,1.4,4.5);
